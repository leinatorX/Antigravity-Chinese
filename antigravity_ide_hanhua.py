...
def 生成汉化脚本() -> str:
    ...
    词典 = {
        ...
        "No": "否",
        "Retry": "重试",
        "Ask a quick question without interrupting the main conversation.": "提问一个快速问题，不中断主对话。",
        "Run until the specified goal is completely finished": "运行直到设定的目标彻底完成",
        "Run an instruction on a recurring schedule or as a one-time timer": "以循环计划或一次性定时器的形式运行指令",
        "Invoke a browser agent for web tasks": "调用浏览器智能体执行网络任务",
        "Interview me to align on a plan": "对我进行提问以对齐目标计划",
...
(function () {{
  ...
  function fanYiWenBen(yuanWen) {{
    ...
    const zhengZeTiHuan = [
        ...
        [/^\s*options\.\s*(?:Agent settings and permissions can be further customized below\.?|智能体设置和权限可在下方继续自定义。)/gi, "种安全模式。智能体设置和权限可在下方继续自定义。"],
        ...
        [/List\)/gi, "列表中的命令）"],
        [/list\)/gi, "列表中的命令）"],
        [/^You currently don't have any MCP(?: 服务器)? installed\.?\s*$/gi, "你目前没有安装任何 MCP 服务器。"],
        [/^\s*Add an MCP server above or add a custom one via the MCP Config\.?\s*$/gi, "请在上方添加 MCP 服务器，或通过 MCP 配置添加自定义服务器。"],
        [/^\s*Add an MCP server above\s*$/gi, "请在上方添加 MCP 服务器。"],
        [/(?:插件|Extensions)\s+are\s+packaged\s+collections\s+of\s+skills\s+and\s+MCPs\s+to\s+help\s+the\s+(?:智能体|agent)\s+in\s+Antigravity\s+IDE\s+work\s+with\s+Google\s+developer\s+products\.\s*You\s+can\s+always\s+change\s+your\s+choices\s+in\s+(?:设置|Settings)\.?/gi, "插件是将技能和 MCP 打包的集合，用于帮助 Antigravity IDE 中的智能体更好地工作。你随时可以在“设置”中更改你的选择。"],
        [/^\s*are packaged collections of skills and MCPs to help the\s*$/gi, " 是将技能和 MCP 打包的集合，用于帮助 "],
        [/^\s*in\s+Antigravity\s+IDE\s+work\s+with\s+Google\s+developer\s+products\.\s*You\s+can\s+always\s+change\s+your\s+choices\s+in\s*$/gi, " 在 Antigravity IDE 中使用 Google 开发者产品。你随时可以在 "],
