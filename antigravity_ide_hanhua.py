...
(function () {{
  ...
  function fanYiWenBen(yuanWen) {{
    ...
    const zhengZeTiHuan = [
        ...
        [/List\)/gi, "列表中的命令）"],
        [/list\)/gi, "列表中的命令）"],
        [/^You currently don't have any MCP\s*$/gi, "你目前没有安装任何 MCP "],
        [/^\s*installed\.\s*Add an MCP server above or add a custom one via the MCP Config\.\s*$/gi, "。请在上方添加 MCP 服务器，或通过 MCP 配置添加自定义服务器。"],
        [/^\s*installed\.\s*Add an MCP server above\s*$/gi, "。请在上方添加 MCP 服务器。"],
        [/^are packaged collections of skills and MCPs to help the\s*$/gi, "是将技能和 MCP 打包的集合，用于帮助 "],
